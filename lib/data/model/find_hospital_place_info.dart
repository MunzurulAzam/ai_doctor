...
class FindHospitalsPlaceInfo {
  ...
  FindHospitalsPlaceInfo({
    ...
    this.userRatingsTotal,
    this.formattedPhoneNumber,
    this.internationalPhoneNumber = '16263',
    this.website,
    this.addressComponents,
    this.formattedAddress,
    this.utcOffset,
  ...
  factory FindHospitalsPlaceInfo.fromFirestore(DocumentSnapshot doc, double calculatedDistanceInKm) {
    ...
    if (data['types'] is List) {
      typesList = List<String>.from(data['types'].map((e) => e.toString()));
    } else if (data['types'] is Map) { 
       typesList = data['types'].values.map((v) => v.toString()).toList();
    }
    
    //!<---------------- distance formatted as string
    final String distanceString = '${calculatedDistanceInKm.toStringAsFixed(2)} km'; 
